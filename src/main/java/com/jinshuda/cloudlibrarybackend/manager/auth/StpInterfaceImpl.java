...
import com.jinshuda.cloudlibrarybackend.manager.auth.model.SpaceUserAuthContext;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    public SpaceUserAuthContext getAuthContextByRequest() {
        ...
        if (ContentType.JSON.getValue().equals(header)) {
            // HttpServletRequest默认是只能读取一次地
            String body = ServletUtil.getBody(request);
            spaceUserAuthContext = JSONUtil.toBean(body, SpaceUserAuthContext.class);
        } else {
            Map<String, String> paramMap = ServletUtil.getParamMap(request);
            spaceUserAuthContext = JSONUtil.toBean(JSONUtil.toJsonStr(paramMap), SpaceUserAuthContext.class);
        }
        // 获取请求地址URI
        ...
        // 获取第一个斜线之前地址
        String prefix = StrUtil.subBefore(requestURI, "/", true);
        switch (prefix) {
            case "space":
